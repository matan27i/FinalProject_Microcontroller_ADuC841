...
/* --- Resynchronisation flag ----------------------------------------------
   Set by the main loop when a PESEC_UNCORRECTABLE frame is dropped.  The
   TX advances its current_bus_state on every nibble, so a dropped frame
   leaves rx_previous_bus_state stale by exactly one TX step.  Without
   recovery, the next clean frame's delta would span TWO bus transitions
   and the H1 differential layer would "correct" to a wrong state, making
   the desync permanent.

   When this flag is set, rx_correct_bus_state() adopts the absolutely-
   decoded data as a fresh baseline (PESEC already confirmed S==0+P_OK)
   and clears the flag.  The nibble carried by the dropped frame is lost,
   but no subsequent nibbles are corrupted.
   [MEM] XDATA. */
volatile uint8_t xdata rx_resync_pending = 0;

/* --- H1-layer correction flag --------------------------------------------
   Set by rx_correct_bus_state() whenever the received delta does not
   match the expected minimal codeword and the H1 layer reconstructs the
   bus state from the previous baseline.  Read by the main loop for the
   error LED + UART marker; reset on entry to every rx_correct_bus_state
   call so the flag reflects only the most recent frame. */
volatile uint8_t xdata rx_h1_correction_flag = 0;
...
/* 
   ...
   Parameters:
     ...
     overall_parity  The received overall parity bit (0 or 1) from the
                     26th bus bit, as extracted by the UART framing ISR.
   */
...
uint16_t rx_correct_bus_state(uint16_t corrected_data)
{
    ...
    /* Reset the per-call H1-correction flag so the main loop can read it
       directly after this call to update the LED / UART marker. */
    rx_h1_correction_flag = 0;

    corrected_data &= BUS_STATE_MASK;

    /* --- Resync path -----------------------------------------------------
       A previous frame was dropped (PESEC uncorrectable), so
       rx_previous_bus_state is stale by one TX step.  Trust the absolutely-
       decoded corrected_data (PESEC confirmed S==0 and parity OK -> valid
       codeword), use it as the fresh baseline, and clear the flag.  No H1
       correction is counted because there is no meaningful previous state
       to compare against. */
    if (rx_resync_pending)
    {
        rx_previous_bus_state = corrected_data;
        rx_resync_pending     = 0;
    ...
    {
        /* Single-bit minimal-weight transition: clean H1 step. */
        result = corrected_data;
    }
    else
    {
        /* delta is not a minimal codeword for this syndrome, so the bus
           must have suffered additional bit flips that PESEC missed (e.g.
           a 3+ bit pattern that aliased to a single-error syndrome).
           Reconstruct the bus state by applying expected_delta to the
           previous baseline -- this preserves the nibble at the syndrome
           level even if the absolute state value is wrong. */
        result = (rx_previous_bus_state ^ expected_delta) & BUS_STATE_MASK;
        rx_errors_corrected++;
        rx_h1_correction_flag = 1;
    }
