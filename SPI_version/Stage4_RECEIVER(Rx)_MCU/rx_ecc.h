...
/* Set by main loop on PESEC_UNCORRECTABLE; consumed by rx_correct_bus_state
   on the next frame to skip the differential check and adopt a fresh
   baseline.  See rx_ecc.c for full rationale. */
extern volatile uint8_t xdata rx_resync_pending;

/* Pulsed to 1 by rx_correct_bus_state when the H1 differential layer had
   to reconstruct the bus state.  Reset to 0 on entry to every call.  The
   main loop reads it after the call to drive the error LED + UART marker. */
extern volatile uint8_t xdata rx_h1_correction_flag;
